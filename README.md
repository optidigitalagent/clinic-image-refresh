...
- `TELEGRAM_CHAT_IDS` — ID получателей через запятую или пробелы. Заявка считается
  доставленной, если её получил хотя бы один настроенный получатель.
- `TELEGRAM_CHAT_ID` — устаревший вариант для одного получателя, сохранённый для
